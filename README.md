## 🎯 Viral Hunter Pro – Scraper de Videos Virales de TikTok

Viral Hunter Pro es una **aplicación de escritorio en Python** que te ayuda a encontrar **videos virales de TikTok** para inspirar tu estrategia de contenido.
...
- 🖥️ Interfaz gráfica moderna con **CustomTkinter**, exportación a **Excel** y tarjetas visuales de cada video (miniatura, autor, likes, fecha, keyword).
...
2. La app llama a **Gemini** y genera unas **30 keywords virales**:
   - 10 en español, 10 en inglés, 10 en portugués.
...
## 🛠️ Instalación (paso a paso, con comentarios)

> **Importante:** todos estos pasos se realizan **dentro de la carpeta del proyecto**, donde está `virales_gui_pro.py`.

### 1️⃣ Clonar o descargar el proyecto

Si lo tienes en un `.zip`:
- Descomprime el archivo en alguna carpeta, por ejemplo:  
  `C:\Users\TU_USUARIO\Desktop\Virales tiktok scrapper\Virales lindo`
...
cd "Virales tiktok scrapper/Virales lindo"
```

### 2️⃣ (Recomendado) Crear entorno virtual
...
### 3️⃣ Instalar dependencias de Python (con un solo comando)
...
## 🔑 Configuración de las APIs (Gemini y Apify)
...
En el mismo directorio donde está `virales_gui_pro.py`, crea (o edita) un archivo llamado `config.json` con este contenido:
...
2. **Criterios de Viralidad**
   - `Tema Principal`:
     - Escribe el tema que quieres investigar.  
       Ejemplos:
       - `Ácido hialurónico`
       - `Acné hormonal`
       - `Piel grasa rutina`
   - `Min. Likes`:
     - Número mínimo de likes que debe tener un video para ser considerado (ej: `25000`).
   - `Max. Meses`:
     - Antigüedad máxima de los videos (en meses) para que se consideren actuales (ej: `6`).
...
Puedes modificar el archivo principal `virales_gui_pro.py`:
